...
export class PhProductService {
   ...
   getPhProductEntry(id: number): Observable<PhProductModel> {
   ...
   getPhProductSubmit(data: PhProductModel) {
      data.auditColumns = {
      'userId': 1,
      'hostname': 'test',
      'ipaddress': 'test',
      ...
      switch (data.entryMode) {
          ...
          case 'A': {
          return this.http.post(this._globals.baseAPIUrl + 'PhProduct/create', data, this._cf.requestOptions()).pipe(
          ...
          case 'E': {
          return this.http.post(this._globals.baseAPIUrl + 'PhProduct/edit', data, this._cf.requestOptions()).pipe(
          map((response: Response) => {
          return response.json();
          }), catchError(this._cf.handleError));
          }
          ...
          case 'D': {
          return this.http.post(this._globals.baseAPIUrl + 'PhProduct/delete', data, this._cf.requestOptions()).pipe(
          map((response: Response) => {
          return response.json();
          }), catchError(this._cf.handleError));
          }
